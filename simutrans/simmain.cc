...
{
	LOGE("Debug", "size of structures");

LOGE("sizes", "koord: %d", sizeof(koord));
	LOGE("sizes", "koord3d: %d", sizeof(koord3d));
	LOGE("sizes", "ribi_t::ribi: %d", sizeof(ribi_t::ribi));
	LOGE("sizes", "halthandle_t: %d\n", sizeof(halthandle_t));

	LOGE("sizes", "ding_t: %d", sizeof(ding_t));
	LOGE("sizes", "gebaeude_t: %d", sizeof(gebaeude_t));
	LOGE("sizes", "baum_t: %d", sizeof(baum_t));
	LOGE("sizes", "weg_t: %d", sizeof(weg_t));
	LOGE("sizes", "stadtauto_t: %d\n", sizeof(stadtauto_t));

	LOGE("sizes", "grund_t: %d", sizeof(grund_t));
	LOGE("sizes", "boden_t: %d", sizeof(boden_t));
	LOGE("sizes", "wasser_t: %d", sizeof(wasser_t));
	LOGE("sizes", "planquadrat_t: %d\n", sizeof(planquadrat_t));

	LOGE("sizes", "ware_t: %d", sizeof(ware_t));
	LOGE("sizes", "vehikel_t: %d", sizeof(vehikel_t));
	LOGE("sizes", "haltestelle_t: %d\n", sizeof(haltestelle_t));

	LOGE("sizes", "karte_t: %d", sizeof(karte_t));
	LOGE("sizes", "spieler_t: %d\n", sizeof(spieler_t));
}
...
LOGE( "show_times() simple profiling of drawing routines" );
 	int i;
...
LOGE( "display_img() %i iterations took %li ms", i, dr_time() - ms );
...
LOGE( "display_color_img() with recolor %i iterations took %li ms", i, dr_time() - ms );
...
LOGE( "display_color_img() 3x %i iterations took %li ms", i, dr_time() - ms );
...
LOGE( "display_flush_buffer() %i iterations took %li ms", i, dr_time() - ms );

 	ms = dr_time();
	for (i = 0;  i < 300000;  i++) {
 		display_text_proportional_len_clip(100, 120, "Dies ist ein kurzer Textetxt ...", 0, 0, -1);
	}
LOGE( "display_text_proportional_len_clip() %i iterations took %li ms", i, dr_time() - ms );

 	ms = dr_time();
	for (i = 0;  i < 300000;  i++) {
 		display_fillbox_wh(100, 120, 300, 50, 0, false);
	}
LOGE( "display_fillbox_wh() %i iterations took %li ms", i, dr_time() - ms );

 	ms = dr_time();
 	for (i = 0; i < 2000; i++) {
 		view->display(true);
 	}
LOGE( "view->display(true) %i iterations took %li ms", i, dr_time() - ms );
...
LOGE( "view->display(true) and flush %i iterations took %li ms", i, dr_time() - ms );
...
LOGE( "grund_t::get_neighbour() %i iterations took %li ms", i*weg_t::get_alle_wege().get_count(), dr_time() - ms );
...
LOGE( "welt->sync_step/step(200,1,1) %i iterations took %li ms", i, dr_time() - ms );
}
...
LOGE( "modal_dialogue() called without a display driver => nothing will be shown!" );
...
LOGE( "ask_language No language selected, will use english!" );
...
{
LOGE("sim_new_handler() OUT OF MEMORY");
}
...
#define PATHSEP "/"
	...
	} else if(gimme_arg(argc, argv, "-use_workpath", 1)) {
		
		strcpy(umgebung_t::program_dir,gimme_arg(argc, argv, "-use_workpath", 1));
		strcat( umgebung_t::program_dir, path_sep );		
	}
	else {
	...
	LOGE("Reading low level config data ...\n");
	LOGE("1\n");
	...
	LOGE("Open now simuconf %s\n",path_to_simuconf);
	if(simuconf.open(path_to_simuconf)) {
		{
			LOGE("CONFIG Success Read");
	...
	if (  cli_syslog_enabled  ) {
		LOGE("syslog enabled\n");
		if (  cli_syslog_tag  ) {
			LOGE("Init logging with syslog tag: %s\n", cli_syslog_tag);
			init_logging( "syslog", true, true, version, cli_syslog_tag );
		} else {
			LOGE("Init logging with default syslog tag\n");
	...
	LOGE("look settings.xml finish");
	// continue parsing ...
	chdir( umgebung_t::program_dir );
	if(  found_simuconf  ) {
		if(simuconf.open(path_to_simuconf)) {
			LOGE("parse_simuconf() at config/simuconf.tab: ");
	...
	string obj_conf = string(umgebung_t::user_dir) + "config/simuconf.tab";
	LOGE("open conf: %s",obj_conf.c_str());
	if (simuconf.open(obj_conf.c_str())) {
		LOGE("parse_simuconf() at %s: ", obj_conf.c_str() );
		umgebung_t::default_einstellungen.parse_simuconf( simuconf, disp_width, disp_height, fullscreen, umgebung_t::objfilename );
	}

	LOGE("look simuconf finish");
	...
	if(  gimme_arg(argc, argv, "-server", 0)  ) {
		...
		LOGE( "simmain() Server started on port %i", portadress );
	...
	LOGE( "simmain::main() Version: " VERSION_NUMBER "  Date: " VERSION_DATE);
	LOGE("Debuglevel %i", umgebung_t::verbose_debug);
	LOGE("program_dir %s", umgebung_t::program_dir);
	LOGE("Object path %s", umgebung_t::objfilename.c_str());
	LOGE("home_dir %s", umgebung_t::user_dir);
	LOGE("locale %s", dr_get_locale_string());
//#ifdef DEBUG
...
//#endif
	...
	if (gimme_arg(argc, argv, "-res", 0) != NULL) {
		...
		switch (res) {
			...
			default:
				LOGE("invalid resolution, argument must be 1,2,3 or 4\n"
	...
	if(gimme_arg(argc, argv, "-screensize", 0) != NULL) {
		...
		if (n != 2) {
			LOGE("invalid argument for -screensize option\n"
	...
	LOGE("Preparing display ...");
	LOGE("simmain simgraph_init disp_width=%d, disp_height=%d, fullscreen=%d", disp_width, disp_height, fullscreen);
	simgraph_init(disp_width, disp_height, fullscreen);
	LOGE("simmain .. results in disp_width=%d, disp_height=%d", display_get_width(), display_get_height());
	...
	if (!gimme_arg(argc, argv, "-nosound", 0)  &&  dr_init_sound()) {
		LOGE("Reading compatibility sound data ...");
		sound_besch_t::init();
	}
	else {
		sound_set_mute(true);
	...
	if(  !translator::load(umgebung_t::objfilename)  ) {
		...
		LOGE("simmain::main() Unable to load any language files\n"
	...
	if(  gimme_arg(argc, argv, "-lang", 1)  ) {
		...
		if(  translator::get_language()==-1  ) {
			LOGE("simmain Illegal language defintion \"%s\"", iso );
		}
	...
	LOGE("Reading city configuration ...");
	stadt_t::cityrules_init(umgebung_t::objfilename);

	LOGE("Reading speedbonus configuration ...");
	vehikelbauer_t::speedbonus_init(umgebung_t::objfilename);

	LOGE("Reading menu configuration ...");
	werkzeug_t::init_menu();

	// loading all paks
	LOGE("Reading object data from %s...", umgebung_t::objfilename.c_str());
	...
	if(  umgebung_t::default_einstellungen.get_with_private_paks()  ) {
		...
		if(!obj_reader_t::load(("addons/" + umgebung_t::objfilename).c_str(), translator::translate("Loading addon paks ..."))) {
			LOGE("reading addon object data failed (disabling).\n");
			umgebung_t::default_einstellungen.set_with_private_paks( false );
		}
		chdir( umgebung_t::program_dir );
	}
	...
	if(  new_world  ) {
		...
		if (FILE* const f = fopen(buffer, "rb")) {
			...
			loadgame = buffer;
			fclose(f);
LOGE("simmain loadgame file found at %s",buffer);
		}
	}
...
LOGE("Midi disabled ...");
	...
	if(  loadgame==""  ||  !welt->laden(loadgame.c_str())  ) {
		...
		LOGE("init with default map","(failing will be a pak error!)");
...
LOGE( "Running world, pause=%i, fast forward=%i ... ", welt->is_paused(), welt->is_fast_forward() );
